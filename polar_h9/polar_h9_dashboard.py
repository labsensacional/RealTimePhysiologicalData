#!/usr/bin/env python3
"""
Local web dashboard for Polar H9 live HR/HRV data.
...
RR_WINDOW = 20
BPM_RATE_WINDOW = 8       # samples for derivative (~8s at 1 sample/beat)
RR_MIN_MS = 300           # < 300ms → physically impossible at reported BPM
RR_MAX_MS = 2000          # > 2000ms → missed beat / artifact
RR_ECTOPIC_THRESHOLD = 0.25  # reject if deviates >25% from recent median
RECONNECT_SECONDS = 5
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .status-dot.connected { background: var(--green); box-shadow: 0 0 0 4px rgba(61, 220, 151, .10); }
    .status-dot.error     { background: var(--red);   box-shadow: 0 0 0 4px rgba(255, 107, 107, .10); }
    ...
    .recording-name {
      height: 34px; min-width: 240px; max-width: 360px;
      border: 1px solid rgba(145, 176, 206, .18);
    ...
    main { width: min(960px, 100%); margin: 0 auto; padding: 22px 24px 28px; }
    ...
    .metric .value   { font-size: clamp(26px, 3.5vw, 40px); line-height: 1; font-weight: 750; }
    ...
    .tabs button { height: 28px; padding: 0 10px; border: 0; background: transparent; color: var(--muted); }
    ...
    .raw-toggle.raw-active {
      border-color: rgba(255, 191, 77, .45);
      color: var(--amber);
      background: rgba(255, 191, 77, .06);
    }
    .raw-toggle .dot {
    ...
    canvas { display: block; width: 100%; height: 360px; cursor: crosshair; }
    ...
    .error-text { color: var(--red); margin-top: 12px; overflow-wrap: anywhere; }
    ...
    #marker-popup {
      ...
      border: 1px solid rgba(145, 176, 206, .22);
      border-radius: 10px;
    ...
    .marker-chip .chip-del {
      ...
      font-size: 14px; line-height: 1; padding: 0; height: auto;
...
<body>
  ...
  <main>
    <section class="metrics">
      <article class="metric">
        ...
        <div class="subvalue" id="bpm-rate-sub">Δ --</div>
      </article>
      <article class="metric">
  ...
  <script>
    ...
    const state = {
      ...
      recordingFile: null,
      labels: {
        bpm:          ["Heart Rate",        "bpm",   "#49a2ff"],
        bpm_rate:     ["BPM Δ/s",           "bpm/s", "#b07fff"],
        rr_ms:        ["RR (filtrado)",      "ms",    "#3ddc97"],
        rr_ms_raw:    ["RR (raw)",           "ms",    "#3ddc97"],
    ...
    function toneBpmRate(r) {
      ...
      if (r > 0) return r > 1.5 ? "bad" : "warn";
      return "good";
    }

    // active RR / RMSSD series keys based on rawMode
    ...
    function render(data) {
      ...
      const cs = data.connection_state || "unknown";
      if (cs === "connected") dot.classList.add("connected");
      if (cs === "reconnecting" || cs === "error") dot.classList.add("error");
      $("status").textContent = cs;
      ...
      $("bpm-rate-hint").textContent = Number.isFinite(rate)
        ...
        : "";
      $("bpm-rate-sub").textContent = Number.isFinite(rate)
        ? `Δ ${fmtRate(rate)} bpm/s`
        : "Δ --";
      ...
      $("rr-raw-sub").textContent = Number.isFinite(rrAlt)
      ...
      $("rmssd-raw-sub").textContent = Number.isFinite(rmssdAlt)
    ...
    $("raw-toggle").addEventListener("click", () => {
      ...
      // if active chart series is rr/rmssd, flip it
      if (state.series === "rr_ms" || state.series === "rr_ms_raw")
        state.series = rrKey();
      if (state.series === "rmssd_ms" || state.series === "rmssd_ms_raw")
        state.series = rmssdKey();
    ...
    function chartTimeRange() {
      ...
      if (state.windowSeconds > 0)
        return { xStart: now - state.windowSeconds, xEnd: now };
      if (!state.history.length)
        return { xStart: now - 10, xEnd: now };
    ...
    function drawChart() {
      ...
      // resolve the active series key (respecting raw mode for rr/rmssd)
      ...
      if (seriesKey === "bpm_rate") {
        const values0 = plot.map((s) => s[seriesKey]).filter((v) => Number.isFinite(v));
        if (values0.length >= 2) {
          const vmin0 = Math.min(...values0), vmax0 = Math.max(...values0);
          const span0 = Math.max(1, vmax0 - vmin0);
          const yMin0 = vmin0 - span0 * 0.12, yMax0 = vmax0 + span0 * 0.12;
          if (yMin0 < 0 && yMax0 > 0) {
            const y0 = valToY(0, yMin0, yMax0, h);
            ctx.strokeStyle = "rgba(145,176,206,.25)";
            ctx.setLineDash([4, 4]);
      ...
      // line (color by sign for bpm_rate)
      if (seriesKey === "bpm_rate") {
    ...
    function drawMarkersOnChart(ctx, w, h, xStart, xEnd, yMin, yMax) {
      const allMarkers = pendingMarker ? [...state.markers, pendingMarker] : state.markers;
      allMarkers.forEach((m) => {
        ...
        if (m.label) {
          ...
          ctx.beginPath();
          ctx.roundRect(x - tw/2 - px, PAD.top + 14 - py, tw + px*2, 13 + py*2, 4);
          ctx.fill();
          ctx.globalAlpha = isPending ? 0.5 : 1;
          ctx.fillStyle = c;
    ...
    function closeMarkerPopup() {
      $("marker-popup").classList.remove("open");
      pendingMarker = null;
      drawChart();
    }
...
class PolarCollector:
    def __init__(self, address=None):
        ...
        self.clients = []
        self.rr_buffer = deque(maxlen=RR_WINDOW)        # raw
        self.rr_buffer_clean = deque(maxlen=RR_WINDOW)  # artifact-filtered
    ...
    def update_hr(self, bpm, rr_intervals):
        now = time.time()
        with self.lock:
            self.state["bpm"] = bpm
            ...
            if len(self.bpm_history) >= 2:
                t0, b0 = self.bpm_history[0]
                t1, b1 = self.bpm_history[-1]
                dt = t1 - t0
        ...
        while not self._stop.is_set():
            try:
                ...
                if not self._stop.is_set():
                    self.set_connection_state("reconnecting", str(exc))
    ...
    async def _connect_once(self):
        addr = self.address
        if not addr:
            self.set_connection_state("scanning")
        ...
        def hr_handler(sender, data):
            ...
            self.update_hr(bpm, rr_intervals)
    ...
    def do_POST(self):
        ...
        if path == "/api/markers":
            ...
            try:
                ...
                marker = {}
            markers = self.collector.add_marker(marker)
            self.send_json({"ok": True, "markers": markers})
            return
...
def main():
    parser = argparse.ArgumentParser(description="Polar H9 live dashboard")
    parser.add_argument("address", nargs="?", help="BLE MAC address (optional, will scan if omitted)")
    parser.add_argument("--host", default="127.0.0.1")
    parser.add_argument("--port", type=int, default=8080)
    args = parser.parse_args()

    collector = PolarCollector(args.address)
    DashboardHandler.collector = collector
    server = ThreadingHTTPServer((args.host, args.port), DashboardHandler)

    print(f"Dashboard: http://{args.host}:{args.port}")
    if args.address:
        print(f"Polar H9: {args.address}")
    else:
        print("Polar H9: scanning...")
    collector.start()
    try:
