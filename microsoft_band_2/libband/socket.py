...
class BandSocketError(Exception):
    pass


class BandSocketTimeout(BandSocketError):
    pass


class BandSocketSendError(BandSocketError):
    pass
        ...
        while True:
            try:
                ...
                if "timed out" in str(error).lower():
                    raise BandSocketTimeout(
                        f"Timed out waiting for response on RFCOMM port "
                        f"{self.port}"
                    ) from error
    ...
    def send(self, packet):
        while True:
            ...
            try:
                ...
                if getattr(error, "errno", None) == errno.EFAULT:
                    raise BandSocketSendError(
                        f"Bluetooth send failed with EFAULT on RFCOMM port "
                        f"{self.port}. If the Band is still in OOBE, finish "
                        f"setup from the old Microsoft Band Android app first."
                    ) from error
