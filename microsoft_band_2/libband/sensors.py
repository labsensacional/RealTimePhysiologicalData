...
class SensorReading():
    ...
    def decode_packet(self, packet):
        ...
        (
            ...
            self.missed_samples,
            self.sample_size
        ) = struct.unpack("<IBBH", packet[:self.header_length])
...
class PedometerSensor(SensorReading):
    ...
    def decode_packet(self, packet):
        ...
        self.value = struct.unpack(
            "<I", packet[self.header_length:self.header_length + 4]
        )[0]
...
class PedometerSensorWithDailyValues(PedometerSensor):
    ...
    def decode_packet(self, packet):
        ...
        self.value, self.value_today = struct.unpack(
            "<II", packet[self.header_length:self.header_length + 8]
        )
...
class DistanceSensor(SensorReading):
    ...
    def decode_packet(self, packet):
        ...
        self.value, self.speed, self.pace = (
            struct.unpack(f'<I{"x"*8}IIxx', packet[self.header_length:])
        )
...
class DistanceSensorWithDailyValues(DistanceSensor):
    ...
    def decode_packet(self, packet):
        SensorReading.decode_packet(self, packet)
        (
            self.value,
            self.speed,
            self.pace,
            self.current_motion,
            self.value_today
        ) = struct.unpack('<IIIBI', packet[self.header_length:])
...
class AccelerometerGyroscopeSensor(AccelerometerSensor):
    ...
    def decode_packet(self, packet):
        ...
        vx, vy, vz = struct.unpack(
            '<hhh',
            packet[self.header_length+6:self.header_length+12]
        )
...
class UVSensor(SensorReading):
    ...
    def decode_packet(self, packet):
        super().decode_packet(packet)
        sample = packet[self.header_length:]
        self.value = UVIndexLevel(
            struct.unpack('<H', sample[2:4])[0]
        )
...
class AmbientLightSensor(SensorReading):
    ...
    def decode_packet(self, packet):
        super().decode_packet(packet)
        self.value = struct.unpack(
            '<H', packet[self.header_length:self.header_length + 2]
        )[0]
...
class RRIntervalSensor(SensorReading):
    ...
    def decode_packet(self, packet):
        super().decode_packet(packet)
        self.value = struct.unpack(
            '<xxxxH', packet[self.header_length:]
        )[0] * 0.016592 * 1000
...
class GsrSensor(SensorReading):
    ...
    def decode_packet(self, packet):
        super().decode_packet(packet)
        self.value = struct.unpack('<xIxx', packet[self.header_length:])[0]


class BarometerSensor(SensorReading):
    def get_value_verbose(self):
        return f'{self.air_pressure_hpa:.2f} hPa, {self.temperature_c:.2f} C'

    def decode_packet(self, packet):
        super().decode_packet(packet)
        sample = packet[self.header_length:]
        pressure_raw, temperature_raw = struct.unpack('<IH', sample[:6])
        self.air_pressure_hpa = pressure_raw / 4096.0
        self.temperature_c = temperature_raw / 2048.0
        self.value = self.air_pressure_hpa


class ElevationSensor(SensorReading):
    def get_value_verbose(self):
        return (
            f'total gain: {self.total_gain_cm} cm, '
            f'total loss: {self.total_loss_cm} cm, '
            f'rate: {self.rate_cms:.2f} cm/s'
        )
    ...
    def decode_packet(self, packet):
        ...
        sample = packet[self.header_length:]
        if len(sample) >= 50:
            self.altitude_m = struct.unpack('<f', sample[4:8])[0]
            (
                self.total_gain_cm,
                self.total_loss_cm,
                self.stepping_gain_cm,
                self.stepping_loss_cm,
                self.steps_ascended,
                self.steps_descended,
                self.flights_ascended,
                self.flights_descended,
            ) = struct.unpack('<IIIIIIII', sample[8:40])
            self.rate_cms = struct.unpack('<h', sample[40:42])[0] / 100.0
        else:
            (
                self.total_gain_cm,
                self.total_loss_cm,
                ...
                self.steps_ascended,
                self.steps_descended,
            ) = struct.unpack('<IIIIII', sample[:24])
            self.rate_cms = struct.unpack('<h', sample[24:26])[0] / 100.0
            self.flights_ascended = struct.unpack('<I', sample[28:32])[0]
            self.flights_descended = struct.unpack('<I', sample[32:36])[0]
            self.flights_ascended_today = struct.unpack('<H', sample[36:38])[0]
            self.total_gain_today_cm = struct.unpack('<I', sample[38:42])[0]

        self.value = self.total_gain_cm
...
def decode_sensor_reading(packet):
    ...
    sensor = {
        ...
        Sensor.Barometer: BarometerSensor,
        Sensor.Elevation: ElevationSensor,
        ...
        Sensor.ElevationWithDailyValues: ElevationSensor,
        # TODO: UV With Daily Values
        Sensor.AmbientLightWithDailyValues: AmbientLightSensor,
        Sensor.Gsr200MS: GsrSensor,
